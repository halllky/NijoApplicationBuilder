...
namespace Nijo.Parts.WebClient {
    ...
    internal class MultiViewEditable : IReactPage {
        ...
        SourceFile IReactPage.GetSourceFile() {
            ...
            var loadFeatureUrl = new FindManyFeature(_aggregate).GetUrlStringForReact();
                ...
                RenderContent = () => $$"""
                    ...
                    export default function () {
                      ...
                      const reloadRemoteItems = useCallback(async () => {
                        ...
                        const skip = 0
                        const take = 20
                        const response = await get<AggregateType.{{_aggregate.Item.TypeScriptTypeName}}[]>({{loadFeatureUrl}}, { skip, take })

                        setRemoteItems(response.ok ? response.data : [])

                        dispatchPaging(page => page.loadComplete())
                      }, [ready, loadLocalItems, currentPage, reset, dispatchPaging])
