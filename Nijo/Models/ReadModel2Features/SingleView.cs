...
namespace Nijo.Models.ReadModel2Features {
    ...
    internal class SingleView {
        ...
        public SourceFile GetSourceFile() => new SourceFile {
            ...
            RenderContent = ctx => {
                ...
                return $$"""
                    ...
                    import { {{DefaultUi.USE_CONTEXT}} } from '../../autogenerated-customizer'
                    ...
                    export default function () {
                      ...
                      const browserTitle = React.useMemo(() => {
                        return Customizers?.{{SET_BROWSER_TITLE}}?.(defaultValues) ?? displayName
                      }, [defaultValues, displayName, Customizers?.{{SET_BROWSER_TITLE}}])
                      ...
                      return (
                        <SingleViewPageFrame
                          ...
                          header={<>
                            {Customizers?.{{HEADER_CUSTOMIZER}} && (
                              <Customizers.{{HEADER_CUSTOMIZER}} {...pageState} />
                            )}
                        ...
                        >
                          <{{PAGE_CONTEXT}}.Provider value={pageContextValue}>
                            {Customizers?.{{BODY_CUSTOMIZER}} ? (
                              <Customizers.{{BODY_CUSTOMIZER}} {...pageState} />
                            ) : (
        ...
        private const string BODY_CUSTOMIZER = "BodyCompoent";

        internal string RenderCustomizersDeclaring() {
            var dataClass = new DataClassForDisplay(_aggregate);
            return $$"""
                /** {{_aggregate.Item.DisplayName.Replace("*/", "")}} の詳細画面をカスタマイズします。 */
                {{ComponentPhysicalName}}?: {
                  /** この画面を表示したときのブラウザのタイトルを編集します。 */
                  {{SET_BROWSER_TITLE}}?: (data: ReactHookForm.DefaultValues<AggregateType.{{dataClass.TsTypeName}}> | undefined) => string
                  /** ヘッダ部分（画面タイトルや保存ボタンがあるあたり）に追加されます。 */
                  {{HEADER_CUSTOMIZER}}?: (props: AggregateHook.{{DataHookReturnType}}) => React.ReactNode
                  /** ボディ欄。これが指定されている場合、自動生成されたボディ欄は使用されません。 */
                  {{BODY_CUSTOMIZER}}?: (props: AggregateHook.{{DataHookReturnType}}) => React.ReactNode
                }
                """;
        }
        #endregion カスタマイズ部分
