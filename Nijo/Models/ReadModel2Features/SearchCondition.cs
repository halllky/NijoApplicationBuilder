    ...
    internal class SearchCondition {
        ...
        private void BuildVForm2(FormUIRenderingContext context, Parts.WebClient.VerticalFormSection section) {
            foreach (var m in GetOwnMembers()) {
                ...
                var fullpath = context.GetReactHookFormFieldPath(m.Member);
                var body = m.Member.Options.SearchConditionCustomUiComponentName == null
                    ? m.Member.Options.MemberType.RenderSearchConditionVFormBody(m.Member, context)
                    : $$"""
                        <{{Parts.WebClient.AutoGeneratedCustomizer.CUSTOM_UI_COMPONENT}}.{{m.Member.Options.SearchConditionCustomUiComponentName}} {...{{context.Register}}(`{{fullpath.Join(".")}}`)} readOnly={false} />
                        """;
                section.AddItem(
                    false,
                    m.MemberName,
            ...
            foreach (var m in GetChildMembers()) {
                ...
                if (m.MemberInfo is AggregateMember.Ref @ref
                    && @ref.SearchConditionCustomUiComponentName != null) {

                    // カスタムUIをレンダリングする
                    var fullpath = GetFullPathForRefRHFRegisterName(@ref);
                    var body = $$"""
                        <{{Parts.WebClient.AutoGeneratedCustomizer.CUSTOM_UI_COMPONENT}}.{{@ref.SearchConditionCustomUiComponentName}} {...{{context.Register}}(`{{fullpath.Join(".")}}`)} readOnly={false} />
                        """;
                    section.AddItem(
                        false,
                        m.MemberName,
