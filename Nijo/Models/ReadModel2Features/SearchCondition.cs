            ...
            foreach (var m in renderedMembers.OrderBy(m => m.MemberInfo.Order)) {
                if (m.MemberInfo is AggregateMember.ValueMember vm) {
                    ...
                    var body = vm.Options.SearchConditionCustomUiComponentName == null
                        ...
                        : $$"""
                        <{{AutoGeneratedCustomizer.CUSTOM_UI_COMPONENT}}.{{vm.Options.SearchConditionCustomUiComponentName}} {...{{context.Register}}(`{{fullpath.Join(".")}}`)} readOnly={false} />
                        """;
                    ...
                    var body = $$"""
                        <{{AutoGeneratedCustomizer.CUSTOM_UI_COMPONENT}}.{{@ref.SearchConditionCustomUiComponentName}} {...{{context.Register}}(`{{fullpath.Join(".")}}`)} readOnly={false} />
                        """;
                    section.Append(new VForm2.ItemNode(new VForm2.StringLabel(m.DisplayName), false, body));
