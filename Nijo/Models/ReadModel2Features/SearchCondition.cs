            ...
            foreach (var m in renderedMembers.OrderBy(m => m.MemberInfo.Order)) {
                if (m.MemberInfo is AggregateMember.ValueMember vm) {
                    ...
                    var body = vm.Options.SearchConditionCustomUiComponentName == null
                        ...
                        : $$"""
                        <{{Parts.WebClient.AutoGeneratedCustomizer.CUSTOM_UI_COMPONENT}}.{{vm.Options.SearchConditionCustomUiComponentName}} {...{{context.Register}}(`{{fullpath.Join(".")}}`)} readOnly={false} />
                        """;
                    section.AddItem(
                        false,
                        m.DisplayMemberName,
