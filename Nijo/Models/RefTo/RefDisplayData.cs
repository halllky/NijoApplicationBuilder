    ...
    internal class RefDisplayData {
                    ...
                    if (member is AggregateMember.ValueMember vm) {
                        ...
                        formBuilder.Append(new VForm2.ItemNode(label, vm.Options.WideInVForm, vm.Options.SingleViewCustomUiComponentName == null
                            ? $$"""
                                ...
                                """
                            : $$"""
                                <{{DefaultUi.CUSTOM_UI_COMPONENT}}.{{vm.Options.SingleViewCustomUiComponentName}} {...registerEx2(`{{fullpath.Join(".")}}`)} {{FormUIRenderingContext.READONLY}}={isReadOnlyField2(`{{fullpath.Join(".")}}`)} />
                                """));
                        ...
                        var label = new VForm2.StringLabel(@ref.DisplayName);
                        var fullpath = @ref.GetFullPathAsDataClassForRefTarget();
                        var body = $$"""
                            <{{DefaultUi.CUSTOM_UI_COMPONENT}}.{{@ref.SingleViewCustomUiComponentName}} {...registerEx2(`{{fullpath.Join(".")}}`)} {{FormUIRenderingContext.READONLY}}={isReadOnlyField2(`{{fullpath.Join(".")}}`)} />
                            """;
        ...
        internal string RenderCustomizersDeclaring() {
            return $$"""
                ...
                >(props: {
                  displayName: string
                  name: ReactHookForm.PathValue<TFieldValues, TFieldName> extends (AggregateType.{{TsTypeName}} | undefined) ? TFieldName : never
                  readOnly: boolean
                  required: boolean
                }) => React.ReactNode
                """;
