...
public class ApplicationSchema {
    ...
    private static string GenerateStructuresInfo(IEnumerable<RootAggregate> rootAggregates) {
        ...
        static IEnumerable<string> RenderRootAggregate(RootAggregate rootAggregate) {
            var structures = rootAggregate.EnumerateAutoGeneratedStructures().ToArray();
...
public static class ApplicationSchemaExtensions {
    /// <summary>
    /// デバッグ用。
    /// スキーマ定義上のルート集約とソース自動生成後のアプリケーションで登場する構造体のペアを列挙します。
    /// </summary>
    public static IEnumerable<IInstancePropertyOwnerMetadata> EnumerateAutoGeneratedStructures(this RootAggregate rootAggregate) {
        if (rootAggregate.Model is DataModel) {
            yield return new EFCoreEntity(rootAggregate);
            yield return new SaveCommand(rootAggregate, SaveCommand.E_Type.Update);
