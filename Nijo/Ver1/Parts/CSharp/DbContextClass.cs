...
namespace Nijo.Ver1.Parts.CSharp {
    public class DbContextClass : IMultiAggregateSourceFile {
        ...
        private readonly List<Func<string, string>> _onModelCreating = [];
        ...
        /// Funcの引数はモデルビルダーの変数名
        /// </summary>
        public DbContextClass AddOnModelCreating(Func<string, string> render) {
                ...
                Contents = $$"""
                    ...
                    """)}}
                        ...
                        protected override void OnModelCreating(ModelBuilder modelBuilder) {
                            try {
                                // 集約ごとのモデル定義
                    {{_onModelCreating.SelectTextTemplate(render => $$"""
                                {{WithIndent(render("modelBuilder"), "            ")}}
                    """)}}
