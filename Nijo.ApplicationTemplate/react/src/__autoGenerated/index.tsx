...
function ApplicationRootInContext() {
  ...
  // サイドメニュー
  const { modifySideMenu, modifySideMenuBottom } = useCustomizerContext()
  ...
  const sideMenuBottom = useMemo(() => {
    const defaultMenu: AutoGenerated.SideMenuItem[] = []
    if (AutoGenerated.SHOW_LOCAL_REPOSITORY_MENU) defaultMenu.push({
      url: '/changes',
    ...
    const items = modifySideMenuBottom?.(defaultMenu) ?? defaultMenu
    const tree = Util.toTree(items, { getId: x => x.url, getChildren: x => x.children })
    return Util.flatten(tree)
  }, [changesCount, modifySideMenuBottom])
