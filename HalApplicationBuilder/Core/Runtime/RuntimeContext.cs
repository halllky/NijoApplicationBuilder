...
namespace HalApplicationBuilder.Core.Runtime {
    public class RuntimeContext {
        ...
        public IEnumerable<AutoCompleteSource> AutoComplete(Type runtimeType, string inputText) {
            var aggregate = FindAggregateByRuntimeType(runtimeType);
            var dbEntity = DbSchema.GetDbEntity(aggregate);
            var methodName = new EntityFramework.AutoGeneratedMethods.LoadAutoComplele(dbEntity, Config).MethodName;
            var dbContext = GetDbContext();
            var method = dbContext.GetType().GetMethod(methodName);
            var result = (IEnumerable)method.Invoke(dbContext, new object[] { inputText });

            foreach (var item in result) {
                var key = InstanceKey.Create(item, dbEntity);
