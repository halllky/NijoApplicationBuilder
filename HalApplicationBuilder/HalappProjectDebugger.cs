            ...
            try {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        // ビルド
                        await BuildAsync(linkedTokenSource.Token, E_NpmBuild.None);

                        // dotnet run の再実行
                        // ※ npmのリビルドはnpm側でやってくれるのでなにもしない
                        ...
                        var httpResponse = await client.SendAsync(message, linkedTokenSource.Token);
                        if (!httpResponse.IsSuccessStatusCode) {
                            var errorContent = await httpResponse.Content.ReadAsStringAsync(linkedTokenSource.Token);
