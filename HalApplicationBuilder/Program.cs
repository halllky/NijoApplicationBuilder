    ...
    class Program {
        static void Main(string[] args) {
            ...
            foreach (var aggregate in builder.EnumerateRootAggregates()) {
                var view = new Runtime.AspNetMvc.MultiView { RootAggregate = aggregate };
            ...
            foreach (var aggregate in builder.EnumerateRootAggregates()) {
                var view = new Runtime.AspNetMvc.SingleView { RootAggregate = aggregate };
                var filename = Path.Combine(viewDir, view.FileName);
                using var sw = new StreamWriter(filename, append: false, encoding: Encoding.UTF8);
                sw.Write(view.TransformText());
            }
            ...
            foreach (var aggregate in builder.EnumerateRootAggregates()) {
                var view = new Runtime.AspNetMvc.CreateView { RootAggregate = aggregate };
                var filename = Path.Combine(viewDir, view.FileName);
                using var sw = new StreamWriter(filename, append: false, encoding: Encoding.UTF8);
                sw.Write(view.TransformText());
            }
            ...
            using (var sw = new StreamWriter(controllerFile, append: false, encoding: Encoding.UTF8)) {
                var source = new Runtime.AspNetMvc.Controller { AggregateBuilder = builder, Config = config };
                sw.Write(source.TransformText());
            }
