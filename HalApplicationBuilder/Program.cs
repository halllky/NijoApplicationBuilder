    ...
    public class Program {
        ...
        static async Task<int> Main(string[] args) {
            ...
            debug.SetHandler((path, verbose) => {
                ...
                var project = HalappProject.Open(path, cancellationTokenSource.Token, Console.Out, verbose);
                project.AddReferenceToHalappDll();
                project.StartDebugging();
            }, path, verbose);

            fix.SetHandler((path, verbose) => {
                if (!CheckIfToolIsAvailable(cancellationTokenSource.Token, "dotnet", "npm")) return;
                HalappProject
                    .Open(path, cancellationTokenSource.Token, Console.Out, verbose)
