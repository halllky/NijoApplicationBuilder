...
namespace HalApplicationBuilder {
    public class Program {
        ...
        static void Main(string[] args) {
            ...
            Console.WriteLine("コード自動生成: EFメソッド");
            var efMethodSourceDir = "/__local__/20221211_haldoc_csharp/haldoc/HalApplicationBuilderSampleMvc/EntityFramework";
            var efMethodSourceFile = Path.Combine(efMethodSourceDir, "__AutoGeneratedMethods.cs");
            if (!Directory.Exists(efMethodSourceDir)) Directory.CreateDirectory(efMethodSourceDir);
            using (var sw = new StreamWriter(efMethodSourceFile, append: false, encoding: Encoding.UTF8)) {
                var source = new EntityFramework.AutoGeneratedMethods();
                sw.Write(source.TransformText(
                    service.GetRequiredService<Core.IApplicationSchema>(),
                    service.GetRequiredService<Core.DBModel.IDbSchema>(),
                    service.GetRequiredService<Core.Config>()));
