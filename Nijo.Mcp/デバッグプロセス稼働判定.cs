...
partial class NijoMcpTools {
    ...
    public static async Task<ServiceStatus> デバッグプロセス稼働判定(WorkDirectory workDirectory, TimeSpan timeout) {
        ...
        workDirectory.WriteSectionTitle("デバッグプロセス稼働判定");
        ...
        while (!ready && DateTime.Now < timeoutLimit) {
            if (!dotnetReady) {
                ...
                if (dotnetReady) {
                    workDirectory.WriteToMainLog("[nijo-mcp] ASP.NET Core サーバーの起動を確認しました。");
                } else {
                    workDirectory.WriteToMainLog("[nijo-mcp] ASP.NET Core サーバーはまだ起動されていません。");
                }
            }
            ...
            if (!npmReady) {
                ...
                if (npmReady) {
                    workDirectory.WriteToMainLog("[nijo-mcp] Node.js サーバーの起動を確認しました。");
                } else {
                    workDirectory.WriteToMainLog("[nijo-mcp] Node.js サーバーはまだ起動されていません。");
                }
            }
            ...
            if (!ready) {
                workDirectory.WriteToMainLog("[nijo-mcp] 待機中...");
