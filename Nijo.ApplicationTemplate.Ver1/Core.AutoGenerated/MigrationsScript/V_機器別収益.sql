...
SELECT
    ...
    PD.DESCRIPTION AS 医療機器_機器詳細_説明文,
    ...
    SUM(T2.QUANTITY) AS 利用数量,
    CASE WHEN SUM(T2.QUANTITY) = 0 THEN 0 ELSE SUM(T2.SUBTOTAL) * 1.0 / SUM(T2.QUANTITY) END AS 平均利用単価
FROM
    診療記録 AS T1
