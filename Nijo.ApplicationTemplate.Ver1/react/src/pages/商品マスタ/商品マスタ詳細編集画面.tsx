...
export const 商品マスタ詳細編集画面 = () => {
    ...
    // --- Submit Handler ---
    const onSubmit = useEvent(async (data: AutoGenerated.商品マスタDisplayData) => {
        setErrorMessage(null)
        setIsLoading(true);
        try {
            const method = isNew ? "POST" : "PUT"
            const url = isNew ? "/api/商品マスタ" : `/api/商品マスタ/${id}`
    ...
    const get在庫情報ColumnDefs: Layout.GetColumnDefsFunction<ReactHookForm.FieldArrayWithId<AutoGenerated.商品マスタDisplayData, "在庫情報", "id">> = React.useCallback(cellType => [
        ...
        cellType.text("values.倉庫.倉庫ID", "倉庫ID", { editable: true, required: true }), // 代替：ID直接入力
                ...
                <ReactHookForm.FormProvider {...methods}>
                    <Layout.VForm3.Root>
                        ...
                        <Layout.VForm3.BreakPoint>
                            <Layout.VForm3.Item label="商品ID" required={AutoGenerated.商品マスタConstraints.values.商品ID.required}>
                                <Input.Word name="values.商品ID" control={control} readOnly={!isNew}
                            ...
                            <Layout.VForm3.FullWidthItem label={(
                                <div className="flex flex-row items-center">
                                    ...
                                    <Input.IconButton fill onClick={() => {
                                        const selected = 付属品GridRef.current?.getSelectedRows();
                        ...
                        <Layout.VForm3.FullWidthItem label={(
                            <div className="flex flex-row items-center">
                                ...
                                <Input.IconButton fill onClick={() => {
                                    const selected = 在庫情報GridRef.current?.getSelectedRows();
