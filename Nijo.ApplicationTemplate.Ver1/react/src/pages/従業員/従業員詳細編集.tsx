...
export const 従業員詳細編集 = () => {
  ...
  const methods = ReactHookForm.useForm<AutoGenerated.従業員DisplayData>()
  const { control, handleSubmit, setValue, reset, watch } = methods

  // EditableGrid用のrowSelectionステートを追加
  ...
  // 所属部署を削除する。
  // - データベースから読み込んだデータは willBeDeleted を true にする。
  // - この画面上で新たに追加されたデータは、そのまま削除する。
  const delete所属部署 = useEvent(() => {
    const selectedRows = 所属部署GridRef.current?.getSelectedRows()
    if (!selectedRows) return
    const deletedIndexes: number[] = []
    for (const { rowIndex } of selectedRows) {
      if (所属部署Fields[rowIndex].existsInDatabase) {
        setValue(`所属部署.${rowIndex}.willBeDeleted`, true)
      } else {
        deletedIndexes.push(rowIndex)
      }
    }
    remove所属部署(deletedIndexes)
  })
          ...
          <Layout.VForm3.Root>
            ...
            <Layout.VForm3.FullWidthItem label={(
              <div className="flex flex-row items-center">
                ...
                <Input.IconButton fill onClick={delete所属部署}>削除</Input.IconButton>
              </div>
            )}>
              <Layout.EditableGrid
                ref={所属部署GridRef}
