...
const AggregateFormView = (props: {
  ...
  return (
    <div className={`flex flex-col gap-px ${aggregate.type === "root" ? "p-1" : ""} ${aggregate.type === "child" ? "my-1" : ""}`}>
      {aggregate.type !== "root" && (
  ...
  if (member.type === "own-column") {
    return (
      <label className="flex gap-1 items-center">
        <div className="basis-40 text-sm break-all select-none text-gray-600 text-right">
          {member.columnName}
        </div>
...
const AggregateGridView = (props: {
  ...
  const columnDefs: Layout.GetColumnDefsFunction<ReactHookForm.FieldArrayWithId<SingleViewFormType, string, "id">> = React.useCallback(cellType => {
    const columns: Layout.EditableGridColumnDef<ReactHookForm.FieldArrayWithId<SingleViewFormType, string, "id">>[] = []
    for (const member of childrenAggregate.members) {
      // childrenの中にchildやchildrenがあるケースは未対応
      if (member.type === "root" || member.type === "child" || member.type === "children") {
        continue
      }
      // 親のキーは表示しない
      if (member.type === "parent-key") {
      ...
      if (member.type === "own-column" || member.type === "ref-key") {
        columns.push(cellType.other(member.columnName, {
          isReadOnly: row => row.existsInDb && member.isPrimaryKey,
          renderCell: cell => {
            const isReadOnly = cell.row.original.existsInDb && member.isPrimaryKey
            return (
              <div className={`h-full w-full truncate px-1 ${isReadOnly ? '' : 'bg-white'}`}>
                {cell.row.original.values[member.columnName]}
              </div>
            )
          },
          onStartEditing: e => {
            if (e.row.existsInDb && member.isPrimaryKey) return;
            e.setEditorInitialValue(e.row.values[member.columnName] ?? '')
          },
          onEndEditing: e => {
            if (e.row.existsInDb && member.isPrimaryKey) return;
            const clone = window.structuredClone(e.row)
            clone.values[member.columnName] = e.value
