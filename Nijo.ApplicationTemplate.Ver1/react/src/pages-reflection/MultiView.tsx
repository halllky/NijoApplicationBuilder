...
const serializeSearchCondition = (
  ...
  if (!condition || !(condition as any).filter) return params; // condition.filter が存在しない場合は空のparamsを返す
  ...
  for (const entry of entries) {
    ...
    const value = ReactHookForm.get(filterObject, pathString); // condition.filter から値を取得
...
const deserializeSearchCondition = <TSearchCond extends TSearchConditionGeneric>(
  ...
  // newCondition.filter が存在し、オブジェクトであることを確認・初期化
    ...
    if (paramValue !== null && paramValue !== undefined && paramValue !== '') {
      if (valueMeta.type === 'datetime' || valueMeta.type === 'date') {
        if (dayjs(paramValue, 'YYYY-MM-DD', true).isValid()) {
          ...
          deserializedValue = undefined; // 不正な日付文字列の場合は undefined
        }
      } else if (valueMeta.type === 'int' || valueMeta.type === 'decimal') {
        const num = Number(paramValue);
        deserializedValue = isNaN(num) ? undefined : num; // 数値変換失敗時は undefined
      } else if (valueMeta.type === 'bool') {
        deserializedValue = paramValue.toLowerCase() === 'true';
      } else {
        deserializedValue = paramValue; // 文字列やEnumなど
      }
    } else {
      // URLパラメータが存在しない場合、型に応じたデフォルト値を設定
      if (valueMeta.type === 'datetime' || valueMeta.type === 'date') {
        deserializedValue = ''; // 修正: null から空文字に変更
      } else if (valueMeta.type === 'int' || valueMeta.type === 'decimal') {
        deserializedValue = ''; // 修正: null から空文字に変更
      } else if (valueMeta.type === 'bool') {
        deserializedValue = false; // ブール型は false
      } else {
        deserializedValue = ''; // 文字列型などは空文字
      }
    }

    // undefined 以外の場合に値を設定 (undefined の場合はフォームのデフォルト値が使われることを期待)
    if (deserializedValue !== undefined) {
      ReactHookForm.set(filterObject, pathString, deserializedValue); // newCondition.filter に値を設定
    }
  }
  return newCondition;
};
