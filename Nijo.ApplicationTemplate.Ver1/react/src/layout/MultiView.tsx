...
import { DisplayData, LoadFeature, QueryModelType, SearchCondition } from "../__autoGenerated"
import { GetColumnDefsFunction } from "./EditableGrid/types"
...
export const MultiView = <TQueryModel extends QueryModelType>(
  ...
  const defaultValues = useMemo(() => initialSearchCondition || SearchCondition.create[queryModel](), [initialSearchCondition, queryModel])
  const reactHookFormMethods = useForm<SearchCondition.TypeMap[TQueryModel]>({
    defaultValues: defaultValues as DefaultValues<SearchCondition.TypeMap[TQueryModel]>,
  })
  ...
  // 検索結果
  const [searchResults, setSearchResults] = useState<DisplayData.TypeMap[TQueryModel][]>([])
  ...
  const doSearch = useCallback(async (data: SearchCondition.TypeMap[TQueryModel]) => {
    try {
      setIsSearching(true)
      ...
      // 検索APIの呼び出し
      const response = await complexPost<{ currentPageItems: DisplayData.TypeMap[TQueryModel][], totalCount: number }>(
...
export type MultiViewProps<TQueryModelType extends QueryModelType> = {
  ...
  children?: (props: {
    ...
    reactHookFormMethods: UseFormReturn<SearchCondition.TypeMap[TQueryModelType]>
  }) => React.ReactNode
  /**
   * 画面初期表示時の検索条件。
   * 挙動を安定させるために `useMemo` でラップした値を使用すること。
   */
  initialSearchCondition?: SearchCondition.TypeMap[TQueryModelType]
  /**
   * 列を定義する関数。
   ...
   */
  getColumnDefs: GetColumnDefsFunction<DisplayData.TypeMap[TQueryModelType]>
  /**
   * ヘッダー部に表示する追加ボタン等
   */
