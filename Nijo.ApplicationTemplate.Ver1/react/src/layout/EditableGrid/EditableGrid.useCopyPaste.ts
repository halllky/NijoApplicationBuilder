...
export const useCopyPaste = <TRow extends ReactHookForm.FieldValues,>({
  ...
  const handlePaste: React.ClipboardEventHandler = useCallback(e => {
    if (!activeCell || !rows.length || isEditing || getIsReadOnly(activeCell.rowIndex)) return;
    ...
    try {
      ...
      const calculatedEndRow = startRow + pastedData.length - 1;
      ...
      for (let r = 0; r < rowCount; r++) {
        ...
        const pasteRowIdx = r % pastedData.length;
        const rowData = pastedData[pasteRowIdx];
  ...
  }, [activeCell, rows, columnDefs, isEditing, getIsReadOnly, props, selectedRange]);
