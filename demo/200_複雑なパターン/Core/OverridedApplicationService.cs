...
public partial class OverridedApplicationService : AutoGeneratedApplicationService {
    ...
    public override string? ValidateAvailableCharacter(string? value, string attributeValue, PropertyInfo propertyInfo) {
        if (attributeValue == "半角数字および半角ハイフンのみ") {
            if (string.IsNullOrEmpty(value)) {
                return null; // エラーなし
            }
            foreach (char c in value) {
                if ((c < '0' || c > '9') && c != '-') {
                    return "半角数字および半角ハイフンのみで入力してください。";
                }
            }
            return null; // エラーなし
        }
    ...
    protected override IQueryable<医療機器マスタSearchResult> CreateQuerySource(医療機器マスタSearchCondition searchCondition, IPresentationContext<医療機器マスタSearchConditionMessages> context) {
        return DbContext.医療機器マスタDbSet.Select(e => new 医療機器マスタSearchResult {
            ...
            機器詳細_ExistsInDatabase = e.機器詳細!.Parent_機器ID != null,
            機器詳細_機器説明 = e.機器詳細!.機器説明,
            機器詳細_機器仕様_ExistsInDatabase = e.機器詳細!.機器仕様!.Parent_Parent_機器ID != null,
            機器詳細_機器仕様_重量 = e.機器詳細!.機器仕様!.重量,
            機器詳細_機器仕様_サイズ_ExistsInDatabase = e.機器詳細!.機器仕様!.サイズ!.Parent_Parent_Parent_機器ID != null,
    ...
    protected override IQueryable<保管庫マスタSearchResult> CreateQuerySource(保管庫マスタSearchCondition searchCondition, IPresentationContext<保管庫マスタSearchConditionMessages> context) {
        return DbContext.保管庫マスタDbSet.Select(e => new 保管庫マスタSearchResult {
            保管庫ID = e.保管庫ID,
            保管庫名 = e.保管庫名,
            住所_ExistsInDatabase = e.保管庫マスタの住所!.Parent_保管庫ID != null,
    ...
    protected override IQueryable<措置結果SearchResult> CreateQuerySource(措置結果SearchCondition searchCondition, IPresentationContext<措置結果SearchConditionMessages> context) {
        return DbContext.措置結果DbSet.Select(e => new 措置結果SearchResult {
            ...
            次回措置_ExistsInDatabase = e.次回措置!.Parent_対象措置_措置ID != null,
    ...
    public override Task Execute医療機器管理Async(医療機器管理ParameterDisplayData parameter, IPresentationContextWithReturnValue<医療機器管理ReturnValueDisplayData, 医療機器管理ParameterMessages> context) {
        // TODO: 医療機器管理コマンドの処理を実装してください。
        return Task.FromResult(new 医療機器管理ReturnValue());
    }

    public override Task Execute簡易診療登録Async(簡易診療登録ParameterDisplayData parameter, IPresentationContextWithReturnValue<簡易診療登録ReturnValueDisplayData, 簡易診療登録ParameterMessages> context) {

        context.ReturnValue.処理結果 = $$"""
            パラメータの{{nameof(parameter.患者ID)}}は{{parameter.患者ID}}でした。
    ...
    // 引数戻り値ともに無いコマンドモデル
    public override Task Executeアプリケーション死活監視Async(IPresentationContext<MessageSetter> context) {
