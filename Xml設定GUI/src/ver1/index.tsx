...
import DataModel定義 from "./pages/DataModel定義"
import QueryModel定義 from "./pages/QueryModel定義"
      ...
      <ReactRouter.Route path="/"
        ...
        children={(
          <>
            ...
            <ReactRouter.Route path="/data-model/:key0" element={<DataModel定義 />} />
            <ReactRouter.Route path="/query-model/:key0" element={<QueryModel定義 />} />
...
const ApplicationFrame = () => {
  ...
  const dataModels = ReactHookForm.useFieldArray({ name: 'DataModel定義', control: form.control })
  const queryModels = ReactHookForm.useFieldArray({ name: 'QueryModel定義', control: form.control })
      ...
      <DialogContextProvider>
        <PanelGroup direction="horizontal" className="h-full w-full">
          <Panel className="flex flex-col bg-color-1 border-r border-color-3" defaultSize={20}>
            ...
            {dataModels.fields.map(model => (
              <ReactRouter.Link key={model.uniqueId} to={`/data-model/${model.uniqueId}`} className="truncate">{model.表示名 ?? 'NO NAME'}</ReactRouter.Link>
            ))}
            {queryModels.fields.map(model => (
              <ReactRouter.Link key={model.uniqueId} to={`/query-model/${model.uniqueId}`} className="truncate">{model.表示名 ?? 'NO NAME'}</ReactRouter.Link>
            ))}
